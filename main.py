...
from tkinter import *
import todo
# tkinter window with 4 buttons, named home, todo, bookCalculator, and about with 4 functions
import mic
...
def renderTodoWindow(root):
    ...
    todoScrollbar = Scrollbar(window)
    ...
    todoListbox.grid(row=1, column=1, sticky=N+S+E+W)
    todoScrollbar.grid(row=1, column=2, sticky=N+S+E+W)
    # create a textbox to add a new todo item
    todoTextbox = Text(window, height=1, width=30)
    # create a button to add a new todo item
    todoButton = Button(window, text="Add", command=lambda: todo.addTodo(todoList, todoTextbox.get("1.0", END), todoListbox))
